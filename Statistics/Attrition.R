...
setwd("C:/Users/Sam/Desktop/Other")
options(digits = 4, show.signif.stars=TRUE)
a <- read.csv(file="C:/Users/Sam/Desktop/Other/CSVFiles/attrition.csv",header=TRUE, sep=",", na.strings="?", stringsAsFactors = TRUE)
str(a)
nrow(a)
summary(a)
...
ncol(updated)
write.csv(updated)
write.csv(updated,'updated.csv')


cor.mat <- cor(updated[, c(1:16)], use="complete.obs")
...
probit <- glm(formula = attrition.fac ~ educ2+ educ3+ educ4+educ5+freqtravel1+raretravel2+female+married+ age+ experience+ yrsatcom+yrsatpos+dailypay+satisfaction+distance,
              data=updated,family=binomial(link="probit"))
summary(probit)
...
logit <- glm(formula = attrition.fac~educ2+educ3+educ4+educ5+freqtravel1+raretravel2+female+married+age+experience+yrsatcom+yrsatpos+dailypay+satisfaction+distance,
             data=updated,family=binomial(link="logit"))
summary(logit)
